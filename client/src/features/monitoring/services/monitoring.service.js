...
export async function getSystemSecurityStatus(getToken) {
  try {
    return await apiRequest('/monitoring/security-status', { getToken });
  } catch (error) {
    console.error('[monitoring.service] getSystemSecurityStatus:', error);
...
export async function getSecurityThreatLogs(limit = 20, getToken) {
  try {
    return await apiRequest('/monitoring/security-logs', {
      getToken,
      query: { limit },
    });
  } catch (error) {
