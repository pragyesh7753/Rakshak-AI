...
import {
  ...
  Activity, Users, Globe, Lock,
  RefreshCw, Filter, Download, MoreVertical,
} from 'lucide-react';
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <div className="relative z-10">
              ...
              <div>
                ...
                <div className="flex items-center gap-4 mt-2">
                  <span className={`text-4xl font-extrabold tracking-tight ${
                    statusData.status === 'Safe'     ? 'text-green-400' :
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                return (
                  <div key={metric.label} className="bg-gray-950/50 border border-gray-800/50 p-4 rounded-2xl">
                    <div className="flex items-center gap-3 mb-2">
                      <MetricIcon className={`w-4 h-4 ${metric.color}`} />
                      <span className="text-xs text-gray-500 font-bold uppercase">{metric.label}</span>
                    </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <table className="w-full text-left">
                <thead className="bg-gray-900/50 border-b border-gray-800">
                  <tr className="text-xs text-gray-500 font-bold uppercase tracking-widest">
                    {['Threat Event', 'Target', 'Risk', 'Status', ''].map((h, i) => (
                      <th key={i} className={`px-6 py-4 ${i === 4 ? 'text-right' : ''}`}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-800">
                  {threatLogs.map((log) => (
                    <tr key={log.id} className="hover:bg-gray-900/40 transition-colors group">
                      ...
                      <td className="px-6 py-5">
                        <span className={`px-3 py-1 rounded-full border text-[10px] font-black uppercase ${getRiskBadgeClasses(log.risk)}`}>
                          {log.risk}
                        </span>
                      </td>
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-2 text-xs font-bold text-gray-300">
                          <div className={`w-2 h-2 rounded-full ${log.status === 'Active' ? 'bg-red-500' : 'bg-cyan-500'}`} />
                          {log.status}
                        </div>
                      </td>
                      <td className="px-6 py-5 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button className="px-3 py-1 bg-red-500/10 text-red-500 text-[10px] font-bold rounded-lg border border-red-500/20 hover:bg-red-500 hover:text-white transition-all">
                            BLOCK
                          </button>
                          <button className="p-2 hover:bg-gray-900 rounded-lg text-gray-500">
        ...
        <div className="space-y-6">
            ...
            {statusData.suspiciousIPs.map((ip, i) => (
              <div key={i} className="flex items-center justify-between p-4 bg-gray-950/50 border border-gray-800/80 rounded-2xl hover:border-gray-700 transition-all group">
                ...
                <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-md border ${
                  ip.risk === 'Critical' ? 'text-red-400 border-red-500/20' :
                  ip.risk === 'High'     ? 'text-orange-400 border-orange-500/20' :
          ...
          {/* Emergency Lockdown */}
          <div className="bg-linear-to-br from-red-500/10 to-transparent border border-red-500/20 rounded-3xl p-6">
