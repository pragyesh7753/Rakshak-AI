...
import { getSimilarDomains, getDomainActivities, getGlobalDomainActivities } from '@/lib/supabaseClient';
import { Globe, AlertTriangle, CheckCircle, ExternalLink, ShieldAlert, Clock, ChevronRight, ChevronDown, ListFilter, Terminal } from 'lucide-react';
                ...
                <div className="space-y-3">
                    {domains.map((domain) => (
                        <button
                            ...
                            className={`w-full text-left p-4 rounded-xl border transition-all ${selectedDomain?.id === domain.id
                        ...
                        >
                            <div className="flex items-start justify-between mb-2">
                                ...
                                {domain.similarity_score >= 0.9 && (
                                    <span className="flex-shrink-0 bg-red-500/10 text-red-500 text-[10px] px-2 py-0.5 rounded border border-red-500/20 font-bold uppercase tracking-wider">
                                        High Similarity
                                    </span>
                                )}
                            </div>
                    ...
                    <h3 className="text-lg font-semibold text-white flex items-center gap-2">
                        ...
                        {selectedDomain && (
                            <span className="text-gray-400 text-sm font-normal ml-2">
                                for {selectedDomain.domain_name}
                            </span>
                        )}
                    </h3>
                </div>
                ...
                {loadingActivities ? (
                    ...
                    <div className="space-y-4">
                        {activities.map((activity) => (
                            <div
                                key={activity.id}
                                className={`p-4 rounded-xl border ${activity.is_suspicious
                                    ? 'bg-gray-900 border-red-500/20'
                                ...
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-3 mb-1">
                                            ...
                                            <span className={`text-[10px] px-2 py-0.5 rounded border font-bold uppercase tracking-wider ${getSeverityColor(activity.severity)}`}>
                                                {activity.severity}
                                            </span>
                                ...
                                <tbody className="divide-y divide-gray-800">
                                    {globalLogs.map((log) => (
                                        <tr key={log.id} className="hover:bg-gray-800/30 transition-colors group">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className={`text-[10px] px-2 py-1 rounded border font-bold uppercase tracking-wider ${getSeverityColor(log.severity)}`}>
                                                    {log.severity}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
