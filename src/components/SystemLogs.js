...
export default function SystemLogs({ logs }) {
  ...
  const parseLogMessage = (message) => {
    // Extract tag like [LIVE], [COMPLETED], [ERROR]
  ...
  const formatTime = (timestamp) => {
    try {
      ...
      // Also show exact time for recent logs
  ...
  const isLiveLog = (status) => {
    return status?.toLowerCase() === 'running' || status?.toLowerCase() === 'processing';
  };
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-lg">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-xs sm:text-sm text-gray-400 mt-1">
              Real-time system activity • {logs.length} entries
            </p>
          </div>
      ...
      <div className="divide-y divide-gray-800 max-h-150 overflow-y-auto bg-black/20">
        {logs.map((log, index) => {
          ...
          const isLive = isLiveLog(log.status);
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-4 mb-2">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        ...
                        {tag && (
                          <span className={`font-bold text-xs ${tagColor}`}>
                            [{tag}]
                          </span>
                        )}
                      </div>
                      <p className="text-gray-300 leading-relaxed text-xs break-words">
                        {message || log.message}
                      </p>
                    </div>

                    <span
                      className={`px-2 sm:px-3 py-1 text-xs font-medium rounded-full border shrink-0 ${getStatusColor(
                        log.status
                      )}`}
                    >
                      {log.status}
      ...
      <div className="p-3 sm:p-4 border-t border-gray-800 bg-black/20">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-xs text-gray-500">
          ...
          <span className="font-mono">Updated {formatTime(new Date().toISOString())}</span>
        </div>
      </div>
    </div>
  );
