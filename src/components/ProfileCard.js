...
export default function ProfileCard({ organization }) {
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-lg overflow-hidden">
      {/* Header */}
      <div className="bg-linear-to-r from-cyan-500 to-blue-500 h-24 sm:h-32"></div>
      ...
      <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2">
                  {organization.keywords.map((keyword, index) => (
                    <span
                      key={index}
                      className="px-3 py-1 bg-gray-800 text-cyan-400 text-xs font-medium rounded-full"
                    >
                      {keyword}
                    </span>
                  ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 pt-4 border-t border-gray-800">
          ...
          <div className="bg-gray-800 rounded-lg p-3 sm:p-4">
            ...
            <p className="text-xs sm:text-sm text-white font-medium uppercase">
              {organization.sector}
            </p>
          </div>
        </div>
